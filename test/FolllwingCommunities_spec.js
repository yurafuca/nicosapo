...
describe("FollowingComunities", () => {
  describe("push回数が1", () => {
    before(done => {
      followingComunities.push(["000000", "111111", "222222", "333333"]);
      done();
    });
    it("#ALL は 最新のコミュニティの id の Array を返却する．", () => {
      const all = followingComunities.query("ALL");
      expect(all).to.eql(["000000", "111111", "222222", "333333"]);
    });
    it("#ONLY_JUST_FOLLOWED はフォローしたばかりの id の Array を返却する．", () => {
      const followed_1 = followingComunities.query("ONLY_JUST_FOLLOWED");
      expect(followed_1).to.eql(["000000", "111111", "222222", "333333"]);
    });
    it("#ONLY_JUST_FOLLOWED はフォローしたばかりの id の Array を返却する．", () => {
      followingComunities.push(["000000", "111111", "666666"]);
      const followed_2 = followingComunities.query("ONLY_JUST_FOLLOWED");
      expect(followed_2).to.eql(["666666"]);
    });
  }),
    describe("push回数が2", () => {
      before(done => {
      ...
      it("#ONLY_JUST_FOLLOWED はフォローしたばかりの id の Array を返却する．", () => {
        const followed_1 = followingComunities.query("ONLY_JUST_FOLLOWED");
        expect(followed_1).to.eql(["444444", "555555"]);
      });
      it("#ONLY_JUST_FOLLOWED はフォローしたばかりの id の Array を返却する．", () => {
        followingComunities.push(["000000", "111111", "666666"]);
        const followed_2 = followingComunities.query("ONLY_JUST_FOLLOWED");

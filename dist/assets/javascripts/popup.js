...
/***/ function(module, exports, __webpack_require__) {
	...
	var Massages = function () {
	    function Massages() {
	        _classCallCheck(this, Massages);
	    }
	
	    _createClass(Massages, null, [{
	        key: 'show',
	        value: function show(type) {
	            ...
	            var message = (0, _jquery2.default)('<div class="message"></div>');
	            message.text(messages[type]);
	
	            var communities = (0, _jquery2.default)('#communities');
	...
	var Loading = function () {
	    ...
	    _createClass(Loading, null, [{
	        ...
	        value: function start() {
	            (0, _jquery2.default)('#communities').empty();
	            (0, _jquery2.default)('#communities').addClass('nowloading');
	        }
	    }, {
	        key: 'done',
	        value: function done() {
	            (0, _jquery2.default)('#communities').removeClass('nowloading');
	        }
	    }]);
	...
	(0, _jquery2.default)(function () {
	    (0, _jquery2.default)(document).on('click', '.tab.non-selected', function () {
	        (0, _jquery2.default)('.tab').removeClass('selected non-selected');
	        (0, _jquery2.default)('.tab').not(this).addClass('non-selected');
	        (0, _jquery2.default)(this).addClass('selected');
	
	        if ((0, _jquery2.default)(this).is('#user-lives')) {
	            loadBroadcasts('user');
	        } else {
	...
	function loadBroadcasts(liveType) {
	    console.info(_Api2.default);
	    console.info(_Api2.default.isLogined);
	    Promise.resolve().then(Loading.start).then(_Api2.default.isLogined).catch(function (e) {
	        Loading.done();
	        Massages.show('NOT_LOGINED');
	        reject();
	    }).then(function () {
	        // const bg = chrome.extension.getBackgroundPage();
	        return _Api2.default.loadCasts(liveType);
	...
	function show(programs, liveType) {
	    ...
	    return new Promise(function (resolve, reject) {
	        ...
	        var isEnableShowReservedProgram = localStorage.getItem('options.showReserved.enable');
	        _jquery2.default.each(programs, function (index, item) {
	            if (!enableOrNull(isEnableShowReservedProgram) && isReserved(item)) {
	                // Do nothing.
	            } else {
	                console.info(item);
	                var program = createOneProgram((0, _jquery2.default)(item), liveType, programs.length, index);
	                (0, _jquery2.default)('#communities').append(program);
	            }
	            if (index == length - 1) {
	                if ((0, _jquery2.default)('#communities').children().length == 0) {
	...
	function isReserved(program) {
	    var is_reserved = (0, _jquery2.default)(program).find('video is_reserved').text();
	...
	function createOneProgram(program, programType, numOfPrograms, currentIndex) {
	    ...
	    if (programType == 'official') {
	        var communityId = (0, _jquery2.default)(program).find('.video_text a').attr('href');
	        var regexp = /http\:\/\/ch.nicovideo.jp\/channel\/(.+)/;
	        var resultarr = regexp.exec(communityId);
	        var thumbnailTmp = undefined;
	
	        if (resultarr != null) {
	            thumbnailTmp = 'http://icon.nimg.jp/channel/' + resultarr[1] + '.jpg';
	        } else {
	            thumbnailTmp = (0, _jquery2.default)(program).find('.info a img').attr('src');
	        }
	
	        attributes.thumbnailUrl = thumbnailTmp;
	        attributes.thumbnailAsProp = 'url(' + attributes.thumbnailUrl + ')';
	        attributes.title = (0, _jquery2.default)(program).find('.video_title').text();
	        attributes.id = 'lv' + (0, _jquery2.default)(program).find('.video_id').text();
	        attributes.url = 'http://live.nicovideo.jp/watch/' + attributes.id;
	    }
	    ...
	    var charPerLine = 16;
	    var wrappedTitle = wordWrap(attributes.title, charPerLine);
	    var tooltipText = wrappedTitle;
	    ...
	    if (isReserved(program) == true) {
	        ...
	        var startTimeInfo = '<span style="color:#adff2f">' + program.find('video open_time_jpstr').text() + '</span><br>';
	        ...
	        var startDayJpn = program.find('video open_time_jpstr').text().match(/\d+\/(\d+)/)[1]; // Month/Day(Date) ...
	        var startDateJpn = program.find('video open_time_jpstr').text().match(/\d+\/\d+\((.)\)/)[1];
	    ...
	    _jquery2.default.fn.powerTip.smartPlacementLists.n = ['n', 's', 'ne', 'nw', 'e', 'w', 'n'];
	    ...
	    if (programType == 'official') {
	        // TODO: Fix.
	        var hasManyCasts = numOfPrograms > 35;
	        var isTopRow = currentIndex < 5;
	        var offset = hasManyCasts && !isTopRow ? -219 : 10;
	        var smartPlacement = currentIndex < 5 ? false : true;
	
	        // TODO: Fix.
	        var manualPositions = ['se', 's', 's', 's', 'sw'];
	        var placement = currentIndex < 5 ? manualPositions[currentIndex] : 'n';
	        console.log(offset);
	...
	function wordWrap(text, length) {
	    var reg = new RegExp("(.{" + parseInt(length) + "})", "g");

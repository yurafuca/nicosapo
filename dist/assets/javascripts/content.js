...
/***/ function(module, exports, __webpack_require__) {
	...
	var _nicovideo = __webpack_require__(4);

	var _common = __webpack_require__(6);

	var _pageType = __webpack_require__(7);

	var _timeCounter = __webpack_require__(8);

	var _formatNicoPage = __webpack_require__(9);

	var _idHolder = __webpack_require__(10);
	...
	var timeCounter = new _timeCounter.TimeCounter(new Date());
	var formatNicoPage = new _formatNicoPage.FormatNicoPage();
	var idHolder = new _idHolder.IdHolder();

	var autoRedirectButton = new _autoRedirectButton.AutoRedirectButton();
	var autoEnterCommunityButton = new _autoEnterCommunityButton.AutoEnterCommunityButton();
	var autoEnterProgramButton = new _autoEnterProgramButton.AutoEnterProgramButton();

	(0, _jquery2.default)(function () {
	    var pageType = _pageType.PageType.get();
	    ...
	    switch (pageType) {
	        ...
	        case 'MODERN_CAST_PAGE':
	            ...
	            _nicovideo.Napi.getStatusByBroadcast(idHolder.liveId).then(function (response) {
	                ...
	                var endTimeJpn = _common.Time.toJpnString(endDate.getTime());

	                var restTime_Minute = _common.Time.minuteDistance(currentDate, endDate);
	                var restTime_Second = _common.Time.minuteDistanceOfSec(currentDate, endDate);
	...
	function autoRedirect() {
	    if (autoRedirectButton.isToggledOn()) {
	        ...
	        _log.Log.out(idHolder.liveId + ' is enabled auto redirect.');

	        _nicovideo.Napi.isOffAir(idHolder.liveId).then(function (response) {
	            ...
	            // OFFAIR.
	            _nicovideo.Napi.isStartedBroadcast(idHolder.communityId).then(function (response) {
	                // タイマーを無効化
	                (0, _jquery2.default)('#extended-bar .end-time').text("\u653E\u9001\u304C\u7D42\u4E86\u3057\u307E\u3057\u305F");
	                (0, _jquery2.default)('#extended-bar .rest-time').text("\u653E\u9001\u304C\u7D42\u4E86\u3057\u307E\u3057\u305F");

	                _log.Log.out('Napi.isStartedBroadcast', response);
	        ...
	        _log.Log.out(idHolder.liveId + ' is disabled auto redirect.');
	    }
	    ...
	    _createClass(Napi, [{
	            ...
	            var promise = new Promise(function (resolve, reject) {
	                ...
	                posting.done(function (response) {
	                    ...
	                    $.each($(subscribes), function (index, item) {
	                        ...
	                        var video_title = $(item).find('a').first().attr('title');
	                        var video_url = $(item).find('a').first().attr('href');
	                        ...
	                        var video_open_time_jpstr = $(item).find('strong').first().text();
	        ...
	        value: function getFutureOnair(index) {
	            var promise = new Promise(function (resolve, reject) {
	                ...
	                var posting = $.get(endpoint + index);
	                posting.done(function (response) {
	        ...
	        value: function getOfficialOnair() {
	            var promise = new Promise(function (resolve, reject) {
	                ...
	                var posting = $.get(endpoint);
	                posting.done(function (response) {
	                    var official_lives = $(response).find('.ranking_video');
	        ...
	        value: function getStatus(param) {
	            ...
	            return new Promise(function (resolve, reject) {
	                ...
	                var posting = $.get(endpoint + param);
	                var parameter = parameter_nicovideojs.shift();
	                posting.done(function (response) {
	                    var status = $(response).find('getplayerstatus').attr('status');
	        ...
	        value: function isOffAir(broadcastId) {
	            ...
	            return new Promise(function (resolve, reject) {
	                getStatusByBroadcast(broadcastId).then(function (response) {
	                    var errorCode = $(response).find('error code').text();
	                    // OFFAIR or ERROR.
	                    if (errorCode && errorCode !== 'require_community_member') {
	                        ...
	                        var _broadcastId = $(response).find('stream id').text();
	        ...
	        value: function isStartedBroadcast(communityId) {
	            ...
	            return new Promise(function (resolve, reject) {
	                getStatusByCommunity(theCommunityId).then(function (response) {
	                    ...
	                    // OFFAIR or ONAIR.
	                    var endTime = $(response).find('end_time').text();
	...
	var PageType = function () {
	    ...
	    _createClass(PageType, null, [{
	        ...
	        value: function _isStandByPage() {
	            var flag = $('#gates').length === 0 && $('.gate_title').length > 0;
	        ...
	        value: function _isCommunityPage() {
	            var $targetDom = $('table.communityDetail');
	        ...
	        value: function _isChannelPage() {
	            var $targetDom = $('body#channel_top');
...
/***/ function(module, exports) {
	...
	function getCommunityId() {
	    var communityUrl1 = $('meta[property="og:image"]').attr('content');
	    ...
	    var communityUrl2 = $('a.ch_name').attr('href');
	...
	var AutoRedirectButton = function (_Buttons) {
	    ...
	    _createClass(AutoRedirectButton, [{
	        ...
	        value: function toggleOn() {
	            var $link = $('.' + this._className).find('.link');

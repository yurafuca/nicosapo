...
/***/ function(module, exports, __webpack_require__) {
	...
	var _NewArrival = __webpack_require__(2);
	
	var _NewArrival2 = _interopRequireDefault(_NewArrival);
	
	var _CommunityHolder = __webpack_require__(3);
	
	var _CommunityHolder2 = _interopRequireDefault(_CommunityHolder);
	...
	(0, _jquery2.default)(function () {
	    ...
	    setTimeout(function () {
	        setInterval(function () {
	            Promise.resolve().then(autoEnterProgramRoutine).then(autoEnterCommunityRoutine);
	        }, 1000 * 30);
	    }, 1000 * 5);
	
	    var storagedData = JSON.parse(localStorage['autoEnterCommunityList']);
	    for (var id in storagedData) {
	...
	function refresh() {
	        ...
	        _jquery2.default.each(newArrival.get(videoInfos), function (index, infos) {
	            if (comuHolder.isNew(infos)) {
	                ...
	                if (enableOrNull(localStorage.getItem('options.popup.enable'))) {
	                    var communityId = 'co' + (0, _jquery2.default)(infos).find('community id').text();
	                    var liveId = (0, _jquery2.default)(infos).find('video id').text();
	    ...
	    _Api2.default.getCheckList().then(function (idList) {
	...
	function isExistsInAutoLists(communityId, liveId) {
	    ...
	    for (var id in autoEnterCommunityList) {
	        console.info(id);
	        if (id === communityId) {
	            console.info(true);
	            return true;
	    ...
	    for (var _id in autoEnterProgramList) {
	        console.info(_id);
	        if (_id === liveId) {
	            console.info(true);
	            return true;
	...
	chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	    ...
	    if (request.purpose == 'getFromNestedLocalStorage') {
	        var storagedData = void 0;
	        if (localStorage[request.key]) storagedData = JSON.parse(localStorage[request.key]);else storagedData = {};
	    ...
	    if (request.purpose == 'saveToNestedLocalStorage') {
	        var _storagedData = void 0;
	
	        if (localStorage[request.key]) {
	            _storagedData = JSON.parse(localStorage[request.key]);
	        } else {
	            _storagedData = {};
	        }
	
	        _storagedData[request.innerKey] = {};
	        _storagedData[request.innerKey]['state'] = request.innerValue.state;
	        _storagedData[request.innerKey]['thumbnail'] = request.innerValue.thumbnail;
	        _storagedData[request.innerKey]['title'] = request.innerValue.title;
	
	        if (request.innerValue.openDate) {
	            _storagedData[request.innerKey]['openDate'] = request.innerValue.openDate;
	        }
	
	        if (request.innerValue.owner) {
	            _storagedData[request.innerKey]['owner'] = request.innerValue.owner;
	        }
	
	        localStorage[request.key] = JSON.stringify(_storagedData);
	        return;
	    }
	
	    // removeFromNestedLocalStorage
	    if (request.purpose == 'removeFromNestedLocalStorage') {
	        var _storagedData2 = JSON.parse(localStorage[request.key]);
	        console.info('[imanani] Delete storagedData[innerKey] ', _storagedData2[request.innerKey]);
	        delete _storagedData2[request.innerKey];
	        localStorage[request.key] = JSON.stringify(_storagedData2);
	        return;
	    }
	});
	...
	function autoEnterProgramRoutine() {
	    return new Promise(function (resolve, reject) {
	        ...
	        var storagedData = localStorage['autoEnterProgramList'];
	...
	function autoEnterCommunityRoutine() {
	    return new Promise(function (resolve, reject) {
	        ...
	        var storagedData = localStorage['autoEnterCommunityList'];
	...
	function checkAndOpenFreshCast(id, storagedData) {
	    _Api2.default.isStartedBroadcast(id).then(function (result) {
	        if (result.isStarted == true) {
	            var lastState = storagedData[result.communityId]['state'];
	            if (lastState == 'offair') {
	                chrome.tabs.create({
	...
	function showNotification(newInfos) {
	    var options = {
	        type: "basic",
	        title: "放送開始のお知らせ",
	        message: (0, _jquery2.default)(newInfos).first().find('video title').text(),
	        iconUrl: (0, _jquery2.default)(newInfos).first().find('community thumbnail').text()
	    };
	    console.log(newInfos);
	    var id = (0, _jquery2.default)(newInfos).first().find('video id').text();
	    chrome.notifications.create(id, options);
	}
	...
	function removeReservation(videoInfos) {
	    ...
	    _jquery2.default.each((0, _jquery2.default)(videoInfos), function (index, item) {
	        var is_reserved = (0, _jquery2.default)(item).find('video is_reserved').text();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var NewArrival = function () {
	    function NewArrival() {
	        ...
	        _classCallCheck(this, NewArrival);
	    ...
	    _createClass(NewArrival, [{
	        key: 'setSource',
	        value: function setSource(infos) {
	            this.source = infos;
	        }
	        ...
	        key: 'get',
	        value: function get(infos) {
	            if (this.source == null) {
	                // return infos;
	                return [];
	            }
	
	            var newArrives = _jquery2.default.makeArray();
	            var sourceTimes = _jquery2.default.makeArray();
	
	            _jquery2.default.each(this.source, function (index, item) {
	                ...
	                sourceTimes.push((0, _jquery2.default)(item).find('video open_time_jpstr').text());
	            });
	
	            _jquery2.default.each(infos, function (index, info) {
	                var targetTime = (0, _jquery2.default)(info).find('video open_time_jpstr').text();
	                var result = _jquery2.default.inArray(targetTime, sourceTimes);
	                if (result === -1) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ComuHolder = function () {
	    function ComuHolder() {
	        var initial = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
	
	        _classCallCheck(this, ComuHolder);
	
	        this.source = initial;
	    }
	
	    _createClass(ComuHolder, [{
	        key: 'setSource',
	        value: function setSource(videoInfos) {
	            this.source = videoInfos;
	        }
	    }, {
	        key: 'isNew',
	        value: function isNew(info) {
	            if (this.source == null) {
	                // dont show notification.
	                return false;
	            }
	            // alert("hoge");
	            console.info('[ComuHolder]', (0, _jquery2.default)(info));
	            console.info('[ComuHolder]', (0, _jquery2.default)(info).find('community id'));
	            var id = (0, _jquery2.default)(info).find('community id').text();
	            var inarray = _jquery2.default.inArray(parseInt(id), this.source);
